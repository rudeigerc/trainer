...
class S3(utils.ModelProvider):
    ...
    def download_model(self):
        ...
        logging.info(
            f"Downloading model from S3 bucket: {bucket_name}, prefix: {prefix}"
        )
        logging.info("-" * 40)
        ...
        try:
            ...
            for page in pages:
                if "Contents" in page:
                    for obj in page["Contents"]:
                        ...
                        # Only download specific file types for models
                        allowed_extensions = [".json", ".safetensors", ".model"]
                        if not any(
                            key.lower().endswith(ext) for ext in allowed_extensions
                        ):
                            continue
                        ...
                        destination_path = os.path.join(utils.MODEL_PATH, relative_path)
            ...
            logging.error(f"Unexpected error downloading model: {e}")
            raise
